...
int hotkeys_init(void)
{
    ...
    printf("[INFO] Hotkeys registered (evdev, %d keyboard%s):\n",
           s_nfds, s_nfds > 1 ? "s" : "");
    printf("  Win+Shift+D  →  record display + mic  (toggle)\n");
    printf("  Win+Shift+W  →  record webcam + mic   (toggle)\n");
    printf("  Win+Shift+B  →  record both + mic     (toggle)\n");
...
static void handle_ev(int code, int value)
{
    ...
    case KEY_LEFTMETA:  case KEY_RIGHTMETA:
        s_meta  = (value != 0); return;
    case KEY_LEFTSHIFT: case KEY_RIGHTSHIFT:
        s_shift = (value != 0); return;
    }
